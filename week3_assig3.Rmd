---
title: "Week 3 - Predicting Loan Repayment"
author: "Vishal Gupta"
date: "3/28/2019"
...
```{r}
loans = read.csv('~/Downloads/loans.csv')
...
nrow(missing)
sum(missing$not.fully.paid)/nrow(missing)
sum(loans$not.fully.paid)/nrow(loans)
```

- If we remove the missing observations there will be too little remaining data, leading to overfitting in our models. incorrect
...
loans_imputed = read.csv("~/Downloads/loans_imputed.csv")
```
...
```{r}
set.seed(144)
split = sample.split(loans_imputed$not.fully.paid, SplitRatio=0.70)
train = loans_imputed[split==T,]
test = loans_imputed[split==F,]
```
...
```{r}
loanModel = glm(not.fully.paid ~ .,train,family = binomial)
```
...
```{r}
predicted.risk = predict(loanModel,test,type = 'response')
test$predicted.risk = predicted.risk
sum((predicted.risk>0.5)==test$not.fully.paid)/nrow(test)
```

What is the accuracy of the baseline model? Input the accuracy as a number between 0 and 1.
```{r}
sum(0==test$not.fully.paid)/nrow(test)
```
...
```{r}
loanModel2 = glm(not.fully.paid ~ int.rate,train,family = binomial)
max(predict(loanModel2, newdata=test, type="response"))
```
...
```{r}
sum((predict(loanModel2, newdata=test, type="response")>0.5)==T)
```
...
```{r}
ROCRpred = prediction(predict(loanModel2, newdata=test, type="response"), test$not.fully.paid)
as.numeric(performance(ROCRpred, "auc")@y.values)
```
